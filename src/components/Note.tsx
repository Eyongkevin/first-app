import { FC, useReducer, useState, ChangeEvent } from 'react'

type TNote = {
  id: number
  note: string
}

type TAction = {
  type: string
  payload?: any
}

type TActionTypes = {
  ADD: 'ADD'
  UPDATE: 'UPDATE'
  DELETE: 'DELETE'
}

const actionType: TActionTypes = {
  ADD: 'ADD',
  UPDATE: 'UPDATE',
  DELETE: 'DELETE'
}

const initialNotes: TNote[] = [
  {
    id: 1,
    note: 'Note 1'
  },
  {
    id: 2,
    note: 'Note 2'
  }
]

const reducer = (state: TNote[], action: TAction) => {
  switch (action.type) {
    case actionType.ADD:
      return [...state, action.payload]
    case actionType.DELETE:
      return state.filter(note => note.id !== action.payload)
    case actionType.UPDATE: {
      const updatedNote = action.payload
      return state.map((n: TNote) => (n.id === updatedNote.id ? updatedNote : n))
    }
    default:
      return state
  }
}

const Note = () => {
  const [notes, dispatch] = useReducer(reducer, initialNotes)
  const [note, setNote] = useState<string>('')

  const handleSubmit = (e: ChangeEvent<HTMLFormElement>) => {
    e.preventDefault()

    const newNote = {
      id: Date.now(),
      note
    }
    dispatch({ type: actionType.ADD, payload: newNote })
  }
  return (
    <div>
      <h2>Notes</h2>

      <ul>
        {notes.map((n: TNote) => (
          <li key={n.id}>
            {n.note}{' '}
            <button
              onClick={() =>
                dispatch({
                  type: actionType.DELETE,
                  payload: n.id
                })
              }
            >
              X
            </button>
            <button
              onClick={() =>
                dispatch({
                  type: actionType.UPDATE,
                  payload: { ...n, note }
                })
              }
            >
              UPDATE
            </button>
          </li>
        ))}
      </ul>
      <form onSubmit={handleSubmit}>
        <input placeholder="New note" value={note} onChange={e => setNote(e.target.value)} />
      </form>
    </div>
  )
}

export default Note
